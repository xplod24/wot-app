...
def downloader_tanks_icons(event):
...
def downloader_consumables_and_modules(event):
    if os.path.isdir("./assets/modules"):
        i = len(os.listdir("./assets/modules"))
    else:
        i = 0

    assets_tanks_pager = apiWotAssetsDownloader(wotApiAssetsModules, "pl", ["image"])
    total_pages = assets_tanks_pager[1]['meta']['page_total']
    total_images = assets_tanks_pager[1]['meta']['total']
    print(f"Total pages: {total_pages} Total images: {total_images} Current image count: {i}")

    filetree_file = "assets-modules-tree.txt"
    filetree_data = {}
    ...
    if i < total_images:
        while True:
            ...
            assets_page = apiWotAssetsDownloader(wotApiAssetsModules, "pl", ["image"])
            ...
            for tank_id, inner_dict in assets_page[1]['data'].items():
                ...
                for big_image in inner_dict:
                    ...
                    file_to_download = inner_dict[big_image]
                    file_itself = os.path.basename(file_to_download)
                    check_for_file = os.path.isfile(os.path.join("./assets/modules", file_itself))

                    print(f"Item_id: {tank_id}, Image: {file_to_download}")
                    print(f"Filename: {file_itself} Is in folder?: {check_for_file}")
                    ...
                    if check_for_file:
                        ...
                        try:
                            response = requests.get(file_to_download)
                            filepath = os.path.join("./assets/modules", file_itself)
                            with open(filepath, 'wb') as f:
                                f.write(response.content)
